...
module AccountHelper
  def sandbox
    ...
    #
    # source = open(Article.where(:feed_id => 3).first.url).read
    # doc = Readability::Document.new(source, :tags => %w[div p])
    # doc.prepare_candidates
    # doc.html.to_s.html_safe
    enc_uri = CGI.escape(Article.first.url)
    p "http://www.readability.com/m?url=#{enc_uri}"
